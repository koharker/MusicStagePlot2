				...
				if(rows[row] > 1) {
					...
					if (vc > 0) {
						if (chairs[row][i].shape === "cello") {
							...
							var t = -1 * (-1 * arc_length / 2 + ((angle_step - (angle_step * 0.1 / (rows.length - 1))) * (i-1)) + vcStep);
						} else if (i > vcLoc && i !== (vcLoc + 1)) {
							console.log(i + "chair " + vcLoc + "vcLoc")
							var t = -1 * (-1 * arc_length / 2 + ((angle_step - (angle_step * 0.1 / (rows.length - 1)))* i) + vcStep);
						} else if (i  === (vcLoc +1)) {
							var t = -1 * (-1 * arc_length / 2 + ((angle_step - (angle_step * 0.1 / (rows.length - 1)))* i) + vcStep);
						} else {
							console.log(i + "chair '<' " + vcLoc + "vcLoc")
							var t = -1 * (-1 * arc_length / 2 + ((angle_step - (angle_step * 0.1 / (rows.length - 1)))* i));
							/*for (var j = i; j >= 0; j--) {
