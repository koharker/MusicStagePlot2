...
/*	$('#guide_canvas').dblclick(function() {
...
function decode(code) {
	...
	if(matches != null && matches.length > 1) {
		...
		for(var i = 0; i < matches[1].length; i+= 2) {
			...
			for(var j = 0; j < parseInt(val, 10); j++) {
				chairs[i/2][j] = { enabled: true, x: 0, y: 0, label: false, fontSize: false, type: "sqr" }; // add "type"
			}
		}
	}
	...
	// Bass Stools
	var matches = code.match(/,B([^,]*)/);
	if(matches != null && matches.length > 1) {
		var stools = matches[1];
		for(var i = 0; i < stools.length; i += 4) {
			var row   = parseInt(stools.substring(i  , i+2), 10);
			var chair = parseInt(stools.substring(i+2, i+4), 10);
			chairs[row][chair].type	= "stool"
		}
	}
